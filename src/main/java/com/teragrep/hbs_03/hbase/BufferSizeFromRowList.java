 ...
 */
public final class BufferSizeFromRowList {
    ...
    public BufferSizeFromRowList(final List<Row> rows) {
        this(rows, 2.0, (64L * 1024L * 1024L), (2 * 1024 * 1024));
    }

    public BufferSizeFromRowList(final List<Row> rows, final double overheadMultiplier) {
        this(rows, overheadMultiplier, (64L * 1024L * 1024L), (2 * 1024 * 1024));
    ...
    public long bufferSize() {
        ...
        if (estimatedBatchSize < minSize) {
            LOGGER
                    .debug(
                            "Estimate <{}> smaller than minimum size, using minimum <{}> buffer", estimatedBatchSize,
                            minSize
                    );
            estimateOrMinimumSize = minSize;
        }
        else {
            estimateOrMinimumSize = estimatedBatchSize;
        }

        return Math.min(estimateOrMinimumSize, maxSize);
    }
}
