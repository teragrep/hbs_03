...
public final class BlockRangeStreamTest {
    ...
    public void testRange() {
        ...
        final BlockRangeStream blockRangeStream = new BlockRangeStream(1, 10000, maxBatchSize);
        int loops = 0;
        while (blockRangeStream.hasNext()) {
            Block block = blockRangeStream.next();
    ...
    public void testInvalidRange() {
        final long maxBatchSize = 1000;
        final BlockRangeStream blockRangeStream1 = new BlockRangeStream(10, 10, maxBatchSize);
        Assertions.assertFalse(blockRangeStream1.hasNext());

        final BlockRangeStream blockRangeStream2 = new BlockRangeStream(11, 10, maxBatchSize);
        Assertions.assertFalse(blockRangeStream2.hasNext());

        final BlockRangeStream blockRangeStream3 = new BlockRangeStream(0, -10, maxBatchSize);
        Assertions.assertFalse(blockRangeStream3.hasNext());
    }

    @Test
    public void testBlockNotOverMaxId() {
        final long maxBatchSize = 1000;
        final BlockRangeStream blockRangeStream = new BlockRangeStream(1, 1500, maxBatchSize);

        int loops = 0;
        Block lastBlock = null;
        while (blockRangeStream.hasNext()) {
            lastBlock = blockRangeStream.next();
    ...
    public void testCallingNextWhenHasNextFalse() {
        final long maxBatchSize = 1000;
        final BlockRangeStream blockRangeStream = new BlockRangeStream(10, 10, maxBatchSize);
        Assertions.assertFalse(blockRangeStream.hasNext());
        final NoSuchElementException noSuchElementException = Assertions
                .assertThrows(NoSuchElementException.class, blockRangeStream::next);
